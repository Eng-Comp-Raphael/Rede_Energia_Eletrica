...
public class PintorConexoes implements Painter<JXMapViewer> {
    ...
    public enum TipoVertice { SUBESTACAO, POSTE, CASA }

    public static class VerticeVis {
        public String nome; public GeoPosition posicao; public TipoVertice tipo;
        public VerticeVis(String nome, GeoPosition posicao, TipoVertice tipo) {
            this.nome = nome; this.posicao = posicao; this.tipo = tipo;
        }
    }
    ...
    public PintorConexoes(JXMapViewer mapa) {
        this.mapa = mapa; this.vertices = new HashMap<>(); this.arestas = new ArrayList<>();
    }
    ...
    public void removerVertice(String nome) {
        if (vertices.containsKey(nome)) {
            ...
            if (nome.equals(verticeDestaque)) verticeDestaque = null;
        }
    }
    ...
    public void adicionarAresta(String origem, String destino, int peso) {
        for (ConexaoVis c : arestas) {
            if ((c.origem.equals(origem) && c.destino.equals(destino)) ||
                (c.origem.equals(destino) && c.destino.equals(origem))) return; 
        }
    ...
    public Map<String, VerticeVis> getVertices() { return vertices; }
    public void setVerticeDestaque(String nome) { this.verticeDestaque = nome; }
    ...
    public void destacarVertice(String nome) { verticesDestacados.add(nome); }
    ...
    public void setCorDestaque(Color cor) { this.corDestaque = cor; }
    public void setMostrarApenasDestaques(boolean mostrar) { this.mostrarApenasDestaques = mostrar; }
    ...
    public String getVerticeProximo(Point2D pontoTela, JXMapViewer map, int raioClique) {
        ...
        for (Map.Entry<String, VerticeVis> entry : vertices.entrySet()) {
            ...
            if (pontoTela.distance(x, y) <= raioClique) return entry.getKey();
        }
        return null;
    }
    ...
    public void atualizarAnimacao() {
        ...
        if (progressoAnimacao > 1.0) progressoAnimacao = 0.0;
    }
    ...
    public void paint(Graphics2D g, JXMapViewer map, int w, int h) {
        ...
        for (ConexaoVis aresta : arestas) {
            ...
            if (mostrarApenasDestaques) {
                boolean ambosDestacados = verticesDestacados.contains(aresta.origem) && verticesDestacados.contains(aresta.destino);
                if (!isDestacada && !ambosDestacados) continue; 
            }
            ...
            Rectangle bounds = map.getViewportBounds();
            int x1 = (int) (ptOri.getX() - bounds.getX()); int y1 = (int) (ptOri.getY() - bounds.getY());
            int x2 = (int) (ptDes.getX() - bounds.getX()); int y2 = (int) (ptDes.getY() - bounds.getY());

            if (isSemEnergia) {
                g.setColor(Color.RED);
                g.setStroke(new BasicStroke(3));
            } else if (isDestacada) {
                if (corDestaque == Color.MAGENTA) g.setColor(Color.DARK_GRAY);
                else g.setColor(corDestaque); 
                g.setStroke(new BasicStroke(5));
            } else {
                g.setColor(Color.DARK_GRAY); g.setStroke(new BasicStroke(3));
            }
            ...
            if (!isSemEnergia && (!isDestacada || corDestaque == Color.GREEN || corDestaque == Color.MAGENTA)) {
                int currentX = (int) (x1 + (x2 - x1) * progressoAnimacao);
                int currentY = (int) (y1 + (y2 - y1) * progressoAnimacao);
                g.setColor(Color.YELLOW); g.fillOval(currentX - 6, currentY - 6, 12, 12);
            }
        }
        ...
        for (Map.Entry<String, VerticeVis> entry : vertices.entrySet()) {
            ...
            if (mostrarApenasDestaques && !verticesParaMostrar.contains(nomeVertice)) continue;
            ...
            if (v.tipo == TipoVertice.SUBESTACAO) { 
                tamanho = 26; cor = new Color(138, 43, 226); letraBase = "S";
            } else if (v.tipo == TipoVertice.CASA) { 
                tamanho = 14; cor = new Color(30, 144, 255); letraBase = "C";
            }
    ...
    private static class ConexaoVis {
        String origem; String destino; int peso;
        public ConexaoVis(String origem, String destino, int peso) { 
            this.origem = origem; this.destino = destino; this.peso = peso;
        }
    }
}